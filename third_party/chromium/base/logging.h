...
// ------------
//
// Make a bunch of macros for logging.  The way to log this is to stream
// things to a LOG(<a particular severity level>).  E.g.,
//
...
// compiles.  LOG_IF and development flags also work well togethr
...
//   PLOG_IF(ERROR) << "Couldn't do foo";
//   DPLOG_IF(ERROR) << "Couldn't do foo";
//   PLOG_CHECK(ERROR) << "Couldn't do foo";
//   DPLOG_CHECK(ERROR) << "Couldn't do foo";
//
...
// or vice versa.
#if NDEBUG
#define BaseInitLoggingImpl BaseInitLoggingImpl_build_with_NDEBUG
#else
#define BaseInitLoggingImpl BaseInitLoggingImpl_build_without_NDEBUG
#endif
...
// The default handler shows a dialog box and then terminates the process,
...
// subtle differences between ostream member streaming functions (e.g.,
...
// TODO(akalin): This would be more valuable if there was some way to
...
std::string* MakeCheckOpString<std::string, std::string>(
    const std::string&, const std::string&, const char* names);
#endif
...
#else
// Otherwse, we're a debug build so enable DLOGs and DCHECKs.
...
#define DCHECK_IS_ON() \
  ((::logging::get_dcheck_state() == \
    ::logging::ENABLE_DCHECK_FOR_NON_OFFICIAL_RELEASE_BUIDLS) && \
   LOG_IS_ON(DCHECK))
...
//       statements, there's no guarentee that it will stay closed
//       after this call.
BASE_EXPORT void CloseLogFile();

// Async signal safe logging mechanisms.
